...
class RealMasterExcelTests(unittest.TestCase):
    """実データ（82行・2026-09-14マスター）に対する結合テスト。"""

    @classmethod
    def setUpClass(cls) -> None:
        cls.report = convert_restaurants.convert()

    def test_reads_82_rows(self) -> None:
        self.assertEqual(self.report.total_rows, 82)

    def test_publish_and_exclusion_counts(self) -> None:
        self.assertEqual(len(self.report.published), 75)
    ...
    def test_hours_hidden_for_uncertain_status(self) -> None:
        # 2026-09-14: No.78ぱん茶屋・No.81蔦八を追加したため4→6件
        # （No.24うみの家・No.31ぐらっちぇ・No.67助・No.71三笠・No.78ぱん茶屋・No.81蔦八）
        self.assertEqual(len(self.report.hours_hidden_ids), 6)
